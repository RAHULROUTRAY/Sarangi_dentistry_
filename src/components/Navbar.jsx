...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 flex justify-center transition-all duration-300 ease-in-out px-4 sm:px-6 ${isScrolled ? "pt-4" : "pt-0"}`}>
      <div 
        className={`w-full transition-all duration-300 ease-out relative z-10 ${
          isScrolled 
            ? "max-w-5xl bg-white/95 backdrop-blur-md shadow-lg rounded-full px-5 sm:px-8" 
            : "max-w-7xl bg-transparent shadow-none rounded-full px-5 sm:px-8 lg:px-10"
        }`}
      >
        <div className={`flex items-center justify-between transition-all duration-300 ${isScrolled ? "h-14 md:h-16" : "h-16 md:h-20"}`}>
          ...
          <div className="hidden md:flex items-center space-x-1 lg:space-x-2">
            ...
            <Link
              to="/book-appointment"
              className={`cursor-grow group relative overflow-hidden ml-4 md:ml-6 bg-[#03966a] text-white font-mono font-bold shadow-lg shadow-[#03966a]/20 hover:-translate-y-0.5 transition-all duration-300 shrink-0 flex items-center justify-center ${
                isScrolled 
            ...
            >
              ...
              <span className={`relative z-10 flex items-center justify-center group-hover:text-[#022431] transition-colors duration-500 ${!isScrolled ? "whitespace-nowrap" : ""}`}>
                {isScrolled ? (
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 md:w-5 md:h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
